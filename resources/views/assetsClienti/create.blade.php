<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Aggiungi Asset per ') . $cliente->nome }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6 text-gray-900 rounded-lg">
                <!-- Modulo per creare un nuovo asset -->
                <form action="{{ route('assets.store', $cliente) }}" method="POST">
                    @csrf

                    <div class="mb-4">
                        <label for="nome_assets" class="block text-gray-700 text-sm font-bold mb-2">Nome Asset:</label>
                        <input type="text" name="nome_assets" id="nome_assets" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                    </div>
